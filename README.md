...
The MASTER model now includes temporal uncertainty modeling, which allows it to estimate the uncertainty of its predictions. This is particularly useful for financial time series prediction where uncertainty quantification is important for risk management.

### How It Works

The model outputs:
1. **Mean Prediction**: The expected value of the prediction
2. **Log Variance**: The logarithm of the variance (uncertainty) of the prediction

### Using Uncertainty in Training

Use the `GaussianNLLLoss` from `loss.py` to train the model:
...
### Using Uncertainty in Inference

During inference, the model returns the mean prediction and standard deviation:

```python
mean_pred, std = model(data)
```

You can use these to create confidence intervals around your predictions:

```python
lower_bound = mean_pred - 1.96 * std  # 95% confidence interval lower bound
upper_bound = mean_pred + 1.96 * std  # 95% confidence interval upper bound
```

See `inference_example.py` for examples of how to visualize predictions with uncertainty.

### Benefits

- Better estimation of prediction risk
- More informed decision making
- Identification of regions with high uncertainty where the model is less confident
